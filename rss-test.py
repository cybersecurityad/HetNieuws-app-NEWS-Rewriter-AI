...
def fetch_article_details(link, is_nu_nl=False):
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Successfully fetched article content from {link}")
            if is_nu_nl:
                full_text = extract_nu_nl_article_body(response.text)
            else:
                full_text = extract_generic_article_body(response.text)
            return full_text
        else:
...
def extract_articles(rss_url, num_articles=5, is_nu_nl=False):
    ...
    for entry in feed.entries[:num_articles]:
        ...
        full_text = fetch_article_details(link, is_nu_nl)
...
RSS_FEEDS = {
    ...
    "Volkskrant": {
        "Algemeen nieuws": "https://www.volkskrant.nl/rss.xml",
